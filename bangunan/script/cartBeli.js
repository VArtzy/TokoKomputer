...
var shoppingCart = (function () {
    ...
    // Constructor
    function Item(id, kode, name, price, stok, count) {
        this.id = id
        this.kode = kode
        this.name = name
        this.price = price
    ...
    // Save cart
    function saveCart() {
        setCookie("shoppingCartBeli", JSON.stringify(cart), 7)
    }

    // Load cart
    function loadCart() {
        cart = JSON.parse(getCookie("shoppingCartBeli"))
    }
    if (getCookie("shoppingCartBeli") != null) {
        loadCart()
    }
    ...
    // Add to cart
    obj.addItemToCart = function (id, kode, name, price, stok, count) {
        var item = new Item(id, kode, name, price, stok, count)
        cart.push(item)
        saveCart()
    }

    obj.addItemToCartCount = function (id, kode, name, price, stok, count) {
...
document.querySelectorAll(".add-to-cart").forEach((a) => {
    a.addEventListener("click", (e) => {
        ...
        var kode = Number(a.getAttribute("data-kode"))
        ...
        shoppingCart.addItemToCart(id, kode, name, price, stok, count)
    })
})
...
tambahBarang = (e) => {
    ...
    var kode = Number(e.target.getAttribute("data-kode"))
...
function displayCart() {
    ...
    for (var i in cartArray) {
        ...
        <span>${Number(i) + 1}. ${cartArray[i].name}</span>
        <span class="badge badge-sm">${cartArray[i].kode}</span>
